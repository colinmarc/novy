...
- **Pinout**:
  ...
  - Pin 5: controller presence signal. Actively driven to
    ...
    GND on the bus side.
  - Pin 6: pan detection heartbeat. A ~52 Hz square wave
    (19.4ms period, 4.8ms high / 14.5ms low, 25% duty)
    generated by the controller. Required for the coils
    to perform pan detection — without this signal, the
    coils will not pulse the induction field to check for
    pans, and will not deliver power. Runs continuously
    while the system is active. Generated by the
    controller (confirmed by probing with controller
    disconnected from PSU). The frequency is likely a
    nominal 50 Hz from a free-running oscillator.
...
## Open questions
 ...
 - Do the other power levels work?
 - How stateless is the coil? If I disconnect/reconnect, does
   it maintain power level?
